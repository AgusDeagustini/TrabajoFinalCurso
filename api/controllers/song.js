...
function getSong(req, res){
    ...
    Song.findById(songId).populate({path: 'album'}).exec((err, song) => {
        if(err){
            ...
            if(!song){
                res.status(404).send({message: 'No existe la cancion solicitada'});
            }else{
...
function uploadFile(req, res){
    ...
    if(req.files){
        ...
        if(file_ext == 'mp3' || file_ext == 'wma' || file_ext == 'ogg'){
            ...
            res.status(200).send({message: 'Extension del archivo incorrecta'});
        }
    }else{
        res.status(200).send({message: 'No se ha subido una cancion'});
    }
}
